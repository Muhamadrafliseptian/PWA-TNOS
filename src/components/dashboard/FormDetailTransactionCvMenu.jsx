...
function FormDetailTransactionCvMenu() {
  ...
  const getDataOrder = async () => {
    ...
    setLoader(true);
    const url = `${process.env.REACT_APP_API_PWA}/badan-hukum-cv/get-detail-order/${params.id}`;
  ...
  const onSubmitOrder = async (e) => {
    ...
    try {
      ...
      setLoader(true);
      const url = `${process.env.REACT_APP_API_PWA}/badan-hukum-cv/in-payment`;
      // const url = `http://127.0.0.1:8000/api/badan-hukum-cv/in-payment`;
      await axios
        .post(url, data)
        .then((res) => {
          setLoader(false);
        ...
        <div className="res-class">
          <div className="nav-top-login">
            <Link to="/dashboard" className="btn nav-back-arrow">
              ...
              <h5 className="title-kasnadkw">{t("detail_badan_hukum_cv")}</h5>
            </Link>
          </div>
...
export default FormDetailTransactionCvMenu;
